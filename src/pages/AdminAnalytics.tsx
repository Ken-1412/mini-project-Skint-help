...
interface AnalyticsData {
    ...
    topRestaurants: Array<{ name: string; contributions: number }>;
    recentTrends: Array<{ date: string; meals: number }>;
}
...
export default function AdminAnalytics() {
    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const { count: totalCollected } = await (supabase as any)
                .from('food_packets')
                .select('*', { count: 'exact', head: true });

            // Fetch total food distributed
            const { count: totalDistributed } = await (supabase as any)
                .from('distributions')
                .select('*', { count: 'exact', head: true });

            // Fetch active restaurants
            const { count: activeRest } = await (supabase as any)
                .from('profiles')
                .select('*', { count: 'exact', head: true })
                .eq('role', 'restaurant')
                .eq('status', 'active');
            ...
            const { count: activeWork } = await (supabase as any)
                .from('profiles')
                .select('*', { count: 'exact', head: true })
                .eq('role', 'worker')
            ...
            // Fetch total centers
            const { count: totalCent } = await (supabase as any)
                .from('centers')
                .select('*', { count: 'exact', head: true });

            // Fetch today's distributions
            const today = new Date().toISOString().split('T')[0];
            const { count: todayDist } = await (supabase as any)
                .from('distributions')
                .select('*', { count: 'exact', head: true })
            ...
            const { count: weeklyDist } = await (supabase as any)
                .from('distributions')
                .select('*', { count: 'exact', head: true })
            ...
            const { count: monthlyDist } = await (supabase as any)
                .from('distributions')
                .select('*', { count: 'exact', head: true })
